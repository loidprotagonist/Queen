...
from Loid_ui import *
from Loid_consensus import is_valid_chain, resolve_chain
from Loid_core import MAX_BLOCK_SIZE
...
def receive_tx():
    ...
    banner(f"✅ RECEIVE TX {txid[:64]}")

    result = chain.add_transaction(tx)

    if result["status"] == "accepted":

        banner(f"✅ TX RELAYED {txid[:64]}")
...
def receive_block():
    ...
    banner(f"BLOCK ⛏  #{block.get('block')} RECEIVED")
    ...
    if result["status"] == "accepted":
        ...
        banner(f"BLOCK ACCEPTED ⛏  #{block.get('block')}")
        ...
        banner(f"RELAY BLOCK ⛏  #{block.get('block')}")
...
if __name__ == "__main__":
    ...
    print()
    banner(f"Loid And QUEEN")
    time.sleep(0.5)
    banner(f"NODE START {MY_NODE}")
    time.sleep(0.5)
