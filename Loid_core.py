...
MAX_SUPPLY = 1_000_000 * Loid

INITIAL_REWARD = int(1 * Loid)
...
DIFFICULTY_ADJUST_INTERVAL = 500_000
...
def mine_worker(args):
    ...
    Loid_algorithm, buffer, seed, difficulty, start_nonce, step, result = args
...
class Blockchain:
    ...
    def Loid_algorithm(self, buffer, seed, nonce):
        ...
        for r in range(CPU_ROUNDS):
            ...
            Loid = self.dsha256_bytes(
                Loid +
                chunk +
                nonce_bytes
            )

            buffer[pos:pos+32] = Loid

        QUEEN = Loid

        return self.dsha256(QUEEN)
    ...
    def load_chain(self):
        ...
        if os.path.exists(CHAIN_FILE):
            ...
            if not ok:
                print(f"❌ MISSING Loid And QUEEN: {reason}")
                raise SystemExit(1)
    ...
    def create_genesis(self):
        ...
        coinbase = {
            ...
            "inputs": [],
            "outputs": [{
                "address": "LOID26jAxYn9PPGggKDYw6q44jXsZnL5VDGfZ",
