...
def mine_once(miner):
    ...
    banner("INFO")
    print()
    p(C.W, f"Block      : {height}")
    p(C.W, f"Difficulty : {difficulty}")
    p(C.W, f"Timestamp  : {int(time.time())}")
    target = get_target(difficulty)
    p(C.W, f"Miner      : {miner}")
    p(C.W, f"Target     : {target:064x}")
    print()
    ...
    while not stop.is_set():
        ...
        print(
            f"\r\033[K"
            f"\r{C.Y}⛏ MINING #{height}{C.X} "
            f"{C.Y} NONCE {nonce:,} ".replace(",", ".") +
    ...
    print()
    p(C.W, f"Block        : {height}")
    p(C.W, f"Chain        : {block['chain'][:40]}....")
    p(C.W, f"Timestamp    : {int(time.time())}")
    p(C.W, f"Nonce        : {nonce}")
    p(C.W, f"Miner        : {miner}")
    p(C.W, f"Mining Time  : {total_time:.2f} Seconds")
    p(C.W, f"Block Reward : {reward / Queen:.8f} QUIN")
    fee = 0
    ...
    p(C.W, f"Fee Reward   : {fee / Queen:.8f} QUIN")
    print()
    ...
    if result.get("status") == "accepted":
       ...
       print()
        ...
        p(C.W, "❌ Block Rejected")

        reason = result.get("reason", "Unknown")

        p(C.W, f"Reason : {reason}")

        p(C.W, f"Block  : {block['block']}")
...
def main():
    ...
    while True:
        ...
        banner("MINER MAIN")
        print()
        p(C.W, "1. Mine 1 Block")
        p(C.W, "2. Mine 2 Block")
        p(C.W, "3. Mine 5 Block")
        p(C.W, "4. Mine 10 Block")
        p(C.W, "5. Mine 1M Block")
        p(C.W, "6. Miner Info")
        p(C.W, "7. Exit")
        print()
        ...
        if choice == "1":
            ...
            p(C.W, f"timestamp   : {int(time.time())}")
            p(C.W, f"Reward      : {info['reward'] / Queen:.8f} QUIN")
