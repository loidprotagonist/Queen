...
def main():
    ...
    if not utxos:
        ...
        p(C.K, "❌ No Balance")
        p(C.K, "This wallet has no UTXO.")
        p(C.K, "Mine a block first.")

        return
    balance = sum(u["amount"] for u in utxos if not u.get("spent"))
    balance = get_balance()
    p(C.K, f"PIN         : ??????????")
    p(C.Y, f"Balance     : {balance / Loid:.8f} QUEEN")
    p(C.K, f"Timestamp   : {int(time.time())}")
    print()
    banner("Loid And QUEEN")
    print()
    ...
    print(f"1. Address Book")
    print(f"2. New Address")
    print(f"3. Cancel")
    print()
    ...
    try:
        ...
        p(C.K, "❌ Invalid Amount")
        p(C.K, "Please INPUTS a valid number.")

        return

    print()
    print(f"Message (OPTIONAL) Max 280 characters")
    print(f"Press ENTER skip to Message...")
    print()
    ...
    p(C.W, f"Address    : {ADDRESS}")
    p(C.K, f"Receiver   : {receiver_name}")
    p(C.W, f"Address    : {to}")
    p(C.Y, f"Amount     : {amount / Loid:.8f} QUEEN")
    p(C.Y, f"Fee        : {FEE / Loid:.8f} QUEEN")
    p(C.K, f"Total      : {needed / Loid:.8f} QUEEN")
    ...
    if res.get("status") == "accepted":
        ...
        p(C.K, f"Change    : {change / Loid:.8f} QUEEN")
        print()
